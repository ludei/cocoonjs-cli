...
CordovaPlugin.prototype.executeHook = function(plugin_path, hook_path){
	...
	if(util.inWindows){
		...
		env_vars += ("set CORDOVA_CUSTOM_VERSION='" + new Buffer(CORDOVA_CUSTOM_VERSION).toString('base64') +"' & ");
		...
		env_vars += ("export CORDOVA_CUSTOM_VERSION='" + new Buffer(CORDOVA_CUSTOM_VERSION).toString('base64') +"' && ");
