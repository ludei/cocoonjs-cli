module.exports = function livereload(opt) {
  ...
  // CocoonJS Snippet, modified to make it work in Canvas+
  var src = opt.src || "(location.protocol || \"http:\") + \"//\" + (location.hostname || \"localhost\") + \":\"+" + port + "+\"/livereload.js?snipver=1\"";
  var snippet = "\n<script>eval(' "+
                "if(!window.document.location.reload){" +
                " if(navigator.isCocoonJS) window.document.location.reload = function(){ window.ext[\"IDTK_APP\"].makeCall(\"loadPath\",window.location.href); }"+
                "}" +
